          ...
          <tbody className="divide-y divide-border">
            {requests.map((r) => (
              <tr key={r.id} className="hover:bg-paper-2">
                <td className="px-3 py-2.5 font-medium text-ink">{r.employee.name}</td>
                <td className="px-3 py-2.5 text-slate">{r.employee.clientOrg.name}</td>
                <td className="px-3 py-2.5 capitalize text-slate">{r.type.toLowerCase()}</td>
